...
export class AuthApiService {
  ...
  refreshToken(token: string) {
    return this.authClient.send<object, string>({ cmd: 'refresh token' }, token);
  }

  forgotPass(email) {
      return this.authClient.send<object, string>({ cmd: 'forgot password' }, email)
      .toPromise()
      .catch((err) => { throw new HttpException(err, HttpStatus.FORBIDDEN) });
  }
  ...
  confirmEmail(id: string, code: string) {
      return this.authClient.send<object>({ cmd: 'confirm email' }, { id, code });
  }

  checkCode(id: string, code: string) {
      return this.authClient.send<object>({ cmd: 'check code' }, { id, code });
  }
